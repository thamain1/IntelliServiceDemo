/*
  # Fix Estimate Conversion Priority Enum

  ## Issue
  The conversion functions are using 'medium' for priority, but the ticket_priority
  enum has values: low, normal, high, urgent (no 'medium').

  ## Fix
  Update both functions to use 'normal' instead of 'medium'.

  ## Changes
  - fn_convert_estimate_to_service_ticket: 'medium' → 'normal'
  - fn_convert_estimate_to_project: 'medium' → 'normal' (both places)
*/
...
COMMENT ON FUNCTION fn_convert_estimate_to_service_ticket(uuid, uuid) IS 'Converts accepted estimate to service ticket (SVC type) with normal priority and planned parts/labor/charges';
COMMENT ON FUNCTION fn_convert_estimate_to_project(uuid, uuid) IS 'Converts accepted estimate to project with initial work order (PRJ type, normal priority) and planned parts/labor/charges';
